...
async function regenerateStatsBatch(messageIndices, subject = null, stat = null, greedy = true, toastMessage = '', copyOver = false, scope = null) {
    try {
...
function renderStatsTableHeader(subjects, messageId, adapter) {
...
function renderStatsTableBody(presentStats, subjects, bucket, messageId, adapter) {
...
function getPresentStats(subjects, bucket) {
    const presentStats = subjects.reduce((acc, char) => {
...
function renderStatsTableControls(messageId, container, stats) {
    ...
    addSceneButton.on('click', function () {
        ...
        const b = getScopeAdapter(StatScope.Scene).getBucket(currentStats);
        if (b[name] !== undefined) {
...
function bindStatsTableEditMode(container, stats, messageId, editButton, discardButton) {
    ...
    if (!isEditing) {
        ...
        container.find('table.stats-table').each(function () {
            ...
            const scopeVal = scopeStr ? scopeStr : StatScope.Character;
            ...
            if (adapter.scope === StatScope.Scene) {
                ...
                if (firstTh.length && firstTh.find('.add-scene-btn').length === 0) {
                    const addSceneBtn = $('<i class="fa-solid fa-plus add-scene-btn" title="Add scene to this message"></i>')
                        ...
                        if (!currentStats)
                            return;
                        const b = adapter.getBucket(currentStats);
                        if (b[name] !== undefined) {
                            toastr.info(`Scene '${name}' already exists in this message.`);
                            return;
                        }
        ...
        container.find('table.stats-table').each(function () {
            ...
            table.find('td[data-character]').each(function () {
                ...
                const b = adapter.getBucket(newStats);
                if (!b[char]) {
                    b[char] = {};
                }
...
export function displayStats(messageId, stats, scopes) {
    ...
    const sections = scopes.map(scope => {
        ...
        if (sortedSubjects.length === 0)
            return null;
        return { adapter, bucket, subjects: sortedSubjects };
    }).filter((s) => !!s);
    ...
    for (const section of sections) {
        ...
        table.append(thead);
        const presentStats = getPresentStats(section.subjects, section.bucket);
        renderStatsTableBody(presentStats, section.subjects, section.bucket, messageId, section.adapter).forEach(row => table.append(row));
        if (!controlsAdded) {
