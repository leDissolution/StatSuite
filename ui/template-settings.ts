...
function getTemplateFromElement($element: JQuery<HTMLElement>): { name: string; template: Template } | null {
...
function validateTemplateName(name: string, excludeName?: string): boolean {
    if (!name.trim()) return false;
    if (excludeName && name === excludeName) return true;
    return !Templates.getTemplate(name);
}

function renderTemplatePreview($previewArea: JQuery<HTMLElement>, templateString: string): void {
