...
export class SceneManager {
    ...
    getSceneGraphForMessage(messageId: number) {
        ...
        try {
            const stats = this.getMessageStats(messageId);
            const sceneStats: Record<string, any> | null = (() => {
                const src = stats && (stats as any).Scenes ? (stats as any).Scenes : null;
                if (!src) return null;
                try {
            ...
            if (sceneStats) {
                ...
                const displayNameOf = (id: string): string | null => this.buildDisplayName(scenes, id);

                const parsePassageEntries = (text: string): Array<{ kind: string; target: string; state: string | null }> => {
                    ...
                    for (const seg of text.split(';')) {
                        ...
                        if (!rawTarget || rawTarget.toLowerCase() === 'unspecified') continue;
                        entries.push({ kind, target: rawTarget, state });
                    }
                    return entries;
                };

                const joinPassage = (kind: string, target: string, state: string | null) => {
                    const core = `${kind} to ${target}`.trim();
                    return state ? `${core} ${state}` : core;
                };

                const passagesContainTarget = (text: any, target: string): boolean => {
                ...
                const resolveByName = (name: string): string | null => {
                    ...
                    for (const id of Object.keys(scenes)) {
                        ...
                        if (chain.length < segs.length) continue;
                        const start = chain.length - segs.length;
                        let ok = true;
                        for (let i = 0; i < segs.length; i++) {
                            if (this.ownerlessBase(chain[start + i]!).toLowerCase() !== this.ownerlessBase(segs[i]!).toLowerCase()) { ok = false; break; }
                        }
                ...
                const ensureChain = (segments: string[]): string | null => {
                ...
                for (const [sName, blockAny] of Object.entries(sceneStats)) {
                    ...
                    const passagesRaw = (blockAny as any)?.['passages'];
                    if (!passagesRaw || typeof passagesRaw !== 'string' || passagesRaw.trim() === '') continue;
                    ...
                    for (const e of entries) {
                        ...
                        if (!targetId) {
                            ...
                            const cur = (sceneStats as any)[targetFullName];
                            if (!cur) {
                                (sceneStats as any)[targetFullName] = { passages: reverse };
                            } else {
                                const existing = (cur as any)['passages'] ?? '';
                                if (!passagesContainTarget(existing, originLeaf)) {
                                    (cur as any)['passages'] = existing && String(existing).trim().toLowerCase() !== 'unspecified'
                                        ? `${existing}; ${reverse}`
                                        : reverse;
                                }
                            }
                        ...
                        const cur = (sceneStats as any)[keyName] ?? {};
                        const existing = (cur as any)['passages'] ?? '';
                        if (!passagesContainTarget(existing, originLeaf)) {
                            (cur as any)['passages'] = existing && String(existing).trim().toLowerCase() !== 'unspecified'
                                ? `${existing}; ${reverse}`
                                : reverse;
                            (sceneStats as any)[keyName] = cur;
                        }
                    }
                }
            }
